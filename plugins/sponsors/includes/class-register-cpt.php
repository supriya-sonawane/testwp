...
{
    $value = get_post_meta($post->ID, '_sponsorurl_key', true);
    ?>
    <div class="sponsorlinkdiv">
	    <label for="wporg_field">Sponsor URL Links:</label><br />
	    <input type="text" id="wporg_field" name="sponsorlink_field" placeholder="Enter Sponsor URL" value="<?php echo $value; ?>">
	</div>  
	<style type="text/css">
		#wporg_field {
		    width: 100%;
...
{
    if (array_key_exists('sponsorlink_field', $_POST)) {
        update_post_meta(
            $post_id,
            '_sponsorurl_key',
            $_POST['sponsorlink_field']
        );
    }
}
